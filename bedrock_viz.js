...
var globalCORSWarning = 'If you are loading files from the local filesystem, your browser might not be allowing us to load additional files or examine pixels in maps.  Firefox does not have this limitation.  See README for more info...';
    ...
    try {
        if (enableFlag) {
            ...
            if (fn === undefined || fn.length <= 1) {
                doModal('CORS Error',
                        'Data for elevation image is not available -- see README and re-run<br/>' +
                        '<br/>' +
                        'Hint: You need to run with --html-most (or --html-all)');
                return -1;
            }
...
function setLayer(fn, extraHelp) {
    if (fn.length <= 1) {
        ...
        doModal('Error', 'That image is not available -- see README and re-run.' + extraHelp);
        return -1;
    }
...
function setLayerById(id) {
    var extraHelp = 'You need to run with --html-most (or --html-all)';
    if (0) {
    } else if (id === 1) {
...
function layerGoto(layer) {
    ...
    if (setLayer(dimensionInfo[globalDimensionId].listLayers[layer], 'You need to run with --html-all') === 0) {
        globalLayerMode = 1;
        layerRawIndex = layer;
    ...
    var tour = new Tour({
        ...
        steps: [
            ...
            },
            {
                ...
                placement: 'top',
                title: 'World Name',
                content: 'The name of your world, and the date/time that you ran on it.'
            },
            {
                title: 'About',
                content: ''
